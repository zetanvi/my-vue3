<template>
  <n-layout class="h-screen bg-green-200">
    <div class="w-100 h-full bg-red-300 my-0 mx-auto relative">
      <div
        :style="`height:${cut_count * human_height}px`"
        class="w-50 absolute bottom-0 left-25 bg-yellow-900"
      >
        <div
          v-for="(item, index) in branches"
          :key="index"
          :style="`top:${index * human_height * 2}px`"
          :class="`bg-cyan-400 absolute w-40 h-15 ${
            Math.random() - 0.5 > 0 ? '-left-40' : '-right-40'
          }`"
        >
          {{ item }}
        </div>
      </div>
      <n-button type="primary" size="medium" @click="cutTree">
        砍一下
      </n-button>
    </div>
  </n-layout>
...
const branches = computed(() => {
  return Math.floor(cut_count.value / 2)
})
function star() {
  cut_count.value = 12
